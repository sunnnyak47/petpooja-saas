...
# Durable file storage (logos, menu images). Prefers Supabase Storage; falls
# back to AWS S3; falls back to local disk (ephemeral) if neither is set.
# Supabase: create a PUBLIC bucket, then Project Settings → Storage → S3 keys.
SUPABASE_URL=https://<project-ref>.supabase.co
SUPABASE_S3_ACCESS_KEY_ID=...
SUPABASE_S3_SECRET_ACCESS_KEY=...
SUPABASE_REGION=ap-south-1            # shown in the S3 connection panel
SUPABASE_STORAGE_BUCKET=uploads       # the public bucket name
RAZORPAY_KEY_ID=...
RAZORPAY_KEY_SECRET=...
MSG91_AUTH_KEY=...
