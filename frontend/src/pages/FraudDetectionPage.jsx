...
/* ─── Constants ───────────────────────────────────────────── */
const SEVERITY = {
  critical: { color: 'text-red-600 dark:text-red-400',    bg: 'bg-red-50 dark:bg-red-500/15',       border: 'border-red-200 dark:border-red-500/30',    dot: 'bg-red-500',    label: 'Critical' },
  high:     { color: 'text-orange-600 dark:text-orange-400', bg: 'bg-orange-50 dark:bg-orange-500/15', border: 'border-orange-200 dark:border-orange-500/30', dot: 'bg-orange-500', label: 'High' },
  medium:   { color: 'text-amber-600 dark:text-amber-400',  bg: 'bg-amber-50 dark:bg-amber-500/15',   border: 'border-amber-200 dark:border-amber-500/30',  dot: 'bg-amber-500',  label: 'Medium' },
  low:      { color: 'text-blue-600 dark:text-blue-400',    bg: 'bg-blue-50 dark:bg-blue-500/15',     border: 'border-blue-200 dark:border-blue-500/30',    dot: 'bg-blue-500',   label: 'Low' },
};
...
const RISK_LEVEL = {
  high:   { color: 'text-red-600 dark:text-red-400',     bg: 'bg-red-50 dark:bg-red-500/10',       border: 'border-red-200 dark:border-red-500/20',     label: '🔴 High Risk' },
  medium: { color: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-500/10',   border: 'border-amber-200 dark:border-amber-500/20', label: '🟡 Medium Risk' },
  low:    { color: 'text-blue-600 dark:text-blue-400',   bg: 'bg-blue-50 dark:bg-blue-500/10',     border: 'border-blue-200 dark:border-blue-500/20',   label: '🔵 Low Risk' },
  clean:  { color: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-500/10', border: 'border-emerald-200 dark:border-emerald-500/20', label: '🟢 Clean' },
};
...
function AlertCard({ alert, outletId, onAction }) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-all ${sv.border} ${sv.bg}`}
      style={!alert.is_read ? { outline: '1px solid var(--border)' } : {}}>
      <div className="p-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2">
              <div>
                <div className="flex flex-wrap items-center gap-1.5 mb-1">
                  ...
                  <span className="text-xs text-surface-400 bg-surface-800 px-1.5 py-0.5 rounded">{meta.short}</span>
                  {alert.wa_notified && (
                    <span className="text-xs text-emerald-600 dark:text-green-400 bg-emerald-50 dark:bg-green-500/10 px-1.5 py-0.5 rounded flex items-center gap-1 border border-emerald-200 dark:border-green-500/20">
                      <MessageCircle size={9} /> WA Sent
                    </span>
                  )}
                  {alert.is_resolved && (
                    <span className="text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded flex items-center gap-1 border border-emerald-200 dark:border-emerald-500/20">
                      <CheckCircle size={9} /> Resolved
                    </span>
                  )}
                </div>
                <p className="text-sm font-semibold text-surface-50 leading-snug">{alert.title}</p>
              </div>
            ...
            {Object.keys(ev).length > 0 && (
              <div className="flex flex-wrap gap-1.5 mt-2">
                {ev.cancel_count !== undefined && (
                  <span className="text-xs bg-surface-800 text-surface-400 px-2 py-0.5 rounded-full border border-surface-700">{ev.cancel_count} cancels</span>
                )}
                {ev.total_orders !== undefined && (
                  <span className="text-xs bg-surface-800 text-surface-400 px-2 py-0.5 rounded-full border border-surface-700">{ev.total_orders} total orders</span>
                )}
                ...
                {ev.avg_disc_pct !== undefined && (
                  <span className="text-xs bg-surface-800 text-surface-400 px-2 py-0.5 rounded-full border border-surface-700">avg {ev.avg_disc_pct}% discount</span>
                )}
                {ev.void_count !== undefined && (
                  <span className="text-xs bg-surface-800 text-surface-400 px-2 py-0.5 rounded-full border border-surface-700">{ev.void_count} voids</span>
                )}
                {ev.refund_count !== undefined && (
                  <span className="text-xs bg-surface-800 text-surface-400 px-2 py-0.5 rounded-full border border-surface-700">{ev.refund_count} refunds</span>
                )}
              </div>
            )}
            ...
            {expanded && (
              <div className="mt-3 p-3 bg-surface-800 rounded-lg border border-surface-700">
                <p className="text-xs text-surface-300 leading-relaxed">{alert.description}</p>
                {alert.resolved_note && (
                  <div className="mt-2 p-2 bg-emerald-50 dark:bg-emerald-500/10 rounded border border-emerald-200 dark:border-emerald-500/20">
                    <p className="text-xs text-emerald-600 dark:text-emerald-400">Resolution note: {alert.resolved_note}</p>
                  </div>
                )}
              </div>
            )}
            ...
            {resolving && (
              <div className="mt-3 flex gap-2">
                ...
                <button onClick={() => { onAction('resolve', alert.id, resolveNote); setResolving(false); }}
                  className="px-3 py-1.5 bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 rounded-lg text-xs border border-emerald-300 dark:border-emerald-500/30 hover:bg-emerald-500/30">
                  Confirm
                </button>
                <button onClick={() => setResolving(false)}
                  className="px-3 py-1.5 bg-surface-800 text-surface-400 rounded-lg text-xs border border-surface-700 hover:bg-surface-700">
                  Cancel
                </button>
              </div>
            )}
              ...
              {!alert.is_resolved && (
                <>
                  <button onClick={() => setResolving(!resolving)}
                    className="flex items-center gap-1 text-xs bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2 py-1 rounded-md border border-emerald-200 dark:border-emerald-500/20 hover:bg-emerald-100 dark:hover:bg-emerald-500/20 transition-colors">
                    <Check size={10} /> Resolve
                  </button>
                  <button onClick={() => onAction('dismiss', alert.id)}
                    className="flex items-center gap-1 text-xs bg-surface-800 text-surface-400 px-2 py-1 rounded-md border border-surface-700 hover:bg-surface-700 transition-colors">
    ...
    <div className={`rounded-xl border p-4 ${rl.border} ${rl.bg}`}>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-surface-800 border border-surface-700 flex items-center justify-center text-lg font-bold text-surface-200">
            {staff.name?.[0]?.toUpperCase() || '?'}
          </div>
          <div>
            <div className="font-semibold text-surface-50 text-sm">{staff.name}</div>
            <div className="text-xs text-surface-400 capitalize">{staff.role}</div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        {staff.unresolved > 0 && (
          <span className="text-xs bg-red-50 dark:bg-red-500/20 text-red-600 dark:text-red-400 px-1.5 py-0.5 rounded-full border border-red-200 dark:border-red-500/30">
      ...
      {staff.alert_types?.length > 0 && (
        <div className="flex flex-wrap gap-1">
          {staff.alert_types.map(t => (
            <span key={t} className="text-xs bg-surface-800 text-surface-400 px-1.5 py-0.5 rounded-full border border-surface-700">
    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-surface-900 border border-surface-800 rounded-2xl p-5">
        <div>
          ...
          <p className="text-surface-400 text-sm mt-1">
            AI behavioural analysis · Real-time pattern detection · Silent WhatsApp owner alerts
          </p>
        </div>
        <button onClick={handleScan} disabled={scanning}
          className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-red-600 hover:bg-red-700 transition-colors disabled:opacity-50 shrink-0 shadow-sm">
      ...
      {scanResult && (
        <div className={`rounded-xl border p-4 flex items-center gap-3 ${
          ...
          {scanResult.error ? (
            <><AlertTriangle size={18} className="text-red-500" /><span className="text-sm text-red-600 dark:text-red-400">Scan failed. Check your connection.</span></>
          ) : scanResult.total === 0 ? (
            <><CheckCircle size={18} className="text-emerald-500" /><span className="text-sm text-emerald-600 dark:text-emerald-400">✓ Clean — no new fraud patterns detected.</span></>
          ) : (
            <><AlertTriangle size={18} className="text-amber-500" /><span className="text-sm text-amber-600 dark:text-amber-400">⚠️ Found {scanResult.total} new alert{scanResult.total > 1 ? 's' : ''}. Review below.</span></>
          )}
          <button onClick={() => setScanResult(null)} className="ml-auto text-surface-400 hover:text-surface-200">
            <X size={14} />
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          { label: 'Total Alerts', value: stats?.total || 0,   icon: <ShieldAlert size={16} />, color: 'text-surface-300' },
          { label: 'Unread',       value: stats?.unread || 0,  icon: <Bell size={16} />,        color: 'text-amber-600 dark:text-amber-400' },
          { label: 'Critical',     value: bySev.critical || 0, icon: <AlertTriangle size={16} />, color: 'text-red-600 dark:text-red-400' },
          { label: 'High',         value: bySev.high || 0,     icon: <Flag size={16} />,        color: 'text-orange-600 dark:text-orange-400' },
          { label: 'WA Notified',  value: alertsData?.items?.filter(a => a.wa_notified).length || 0, icon: <MessageCircle size={16} />, color: 'text-emerald-600 dark:text-green-400' },
        ].map(s => (
        ...
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-bold transition-all ${
              tab === t.key ? 'bg-red-600 text-white shadow-md' : 'text-surface-400 hover:text-surface-200'
            }`}>
            {t.icon} {t.label}
          </button>
        ))}
      ...
      {tab === 'alerts' && (
        <>
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <select value={severityFilter} onChange={e => setSeverityFilter(e.target.value)} className="input text-sm py-2">
            ...
            <select value={typeFilter} onChange={e => setTypeFilter(e.target.value)} className="input text-sm py-2">
            ...
            <button onClick={() => setUnreadOnly(!unreadOnly)}
              className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm border transition-colors ${
                unreadOnly
                  ? 'bg-amber-50 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400 border-amber-300 dark:border-amber-500/30'
                  : 'bg-surface-900 text-surface-400 border-surface-800 hover:border-surface-600'
              }`}>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface-900 rounded-2xl border border-surface-800 p-5">
            <h2 className="text-sm font-semibold text-surface-50 mb-4 flex items-center gap-2">
              <BarChart3 size={14} className="text-red-500" /> Alert Types (Last 7 Days)
            </h2>
            {stats?.by_type?.length > 0 ? (
                  ...
                  return (
                    <div key={row.type}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm font-semibold text-surface-50">{row.count}</span>
                      </div>
              ...
              <p className="text-surface-400 text-sm">No alert data yet — run a scan first.</p>
            )}
          </div>
          ...
          <div className="bg-surface-900 rounded-2xl border border-surface-800 p-5">
            <h2 className="text-sm font-semibold text-surface-50 mb-4 flex items-center gap-2">
              <Activity size={14} className="text-amber-500" /> 7-Day Alert Trend
            </h2>
            {stats?.trend_7d?.length > 0 ? (
                ...
                {stats.trend_7d.map((row, i) => {
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      <span className="text-xs text-surface-400">{Number(row.count)}</span>
                      <div className="w-full rounded-t-md bg-gradient-to-t from-red-500/70 to-orange-400/50 transition-all"
                        style={{ height: `${h}%` }} />
              ...
              <p className="text-surface-400 text-sm">No trend data yet.</p>
            )}
          </div>
          ...
          <div className="bg-surface-900 rounded-2xl border border-surface-800 p-5">
            <h2 className="text-sm font-semibold text-surface-50 mb-4 flex items-center gap-2">
              <Shield size={14} className="text-blue-500" /> Detection Rules Active
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {Object.entries(ALERT_TYPE_META).map(([key, meta]) => (
                <div key={key} className="flex items-start gap-3 p-3 bg-surface-800 rounded-xl border border-surface-700">
                  <span className="text-xl mt-0.5">{meta.icon}</span>
                  <div className="flex-1">
                    <div className="text-sm font-medium text-surface-50">{meta.label}</div>
