...
const OnlineStatusBar = () => {
  ...
  // Desktop only: track local orders awaiting cloud sync.
  // Initial count via IPC on mount, then live updates from 'sync-status' events.
  useEffect(() => {
  ...
  return (
    ...
    {/* Desktop pending-sync badge — amber pill, hidden when nothing is queued */}
    {IS_ELECTRON && pendingCount > 0 && (
      <div
        className={`fixed right-3 z-[9998] transition-all duration-500 ease-in-out ${visible ? 'top-11' : 'top-2'}`}
      >
