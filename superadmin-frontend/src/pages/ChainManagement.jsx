...
export default function ChainManagement() {
  ...
  const onboardMutation = useMutation({
    mutationFn: (data) => api.post('/onboard', data),
    onSuccess: () => {
      queryClient.invalidateQueries(['sa-chains']);
  ...
  const impersonateMutation = useMutation({
    ...
    onSuccess: (res) => {
      ...
      if (token) {
        ...
        toast.success('Impersonation token generated! Redirecting...');
        // Optional: window.open('https://petpooja-saas.vercel.app/login?token=' + token, '_blank');
      }
    }
  });

  if (isLoading) return <div className="h-[40vh] flex items-center justify-center font-black text-slate-500 text-lg tracking-widest uppercase text-center">
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-4">
           ...
           <button 
             ...
             className="flex items-center gap-2 px-8 py-4 bg-indigo-600 rounded-2xl text-white font-black hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/20 active:scale-95"
           >
      ...
      <OnboardModal 
        ...
        onOnboard={(data) => onboardMutation.mutateAsync(data)} 
      />
